...
const dst = require('dst-transitions');

const cleanServerUrl = (server) => {
  server = /mqtts?:\/\//.test(server) ? server : 'mqtt://' + server; // add protocol
  server = /:(?:6553[0-5]|655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[1-5][0-9]{4}|[1-9][0-9]{1,3}|[0-9])/.test(
    server
  )
    ? server
...
const serverRegex =
  /((([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])):(?:6553[0-5]|655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[1-5][0-9]{4}|[1-9][0-9]{1,3}|[0-9])$/;
...
const percentToColor = (percent) => (percent > 0.7 ? '^G' : percent > 0.5 ? '^Y' : percent > 0.3 ? '^y' : '^r');
...
module.exports = class API {
  ...
  async deviceInformation() {
    ...
    try {
      const response = await axios.post(`http://${this.host}/config`, packet, {
        headers: {
  ...
  async configureMqttServers(mqtt) {
    ...
    for (let s = 0; s < servers.length; s++) {
      ...
      rows.push([`${s > 0 ? 'Failover' : 'Primary'} MQTT broker`, `${server.host}:${server.port}`]);
    }
  ...
  async getTimeInformation() {
    const fmt = Intl.DateTimeFormat();
    const { timeZone } = fmt.resolvedOptions();

    const transitions = [];
    const d = new Date();
    const currentYear = d.getFullYear();
    for (let year = currentYear; year <= currentYear + 10; year++) {
      const transition = dst.get_transitions(year);

      transitions.push({
        from: new Date(transition.spring.to),
        offset: 1,
      });

      transitions.push({
        from: new Date(transition.fall.to),
        offset: 0,
      });
    }
    ...
    return {
      timezone: timeZone,
      dstRules: transitions,
    };
  }

  async configureTimeZone({ timezone, dstRules }) {
    const timestamp = Math.floor(Date.now() / 1000);
    const packet = this.signPacket({
      header: {
        method: 'SET',
      ...
      payload: {
        time: {
          timestamp,
          timezone,
          timeRule: dstRules.map((rule) => [rule.from.getTime() / 1000, rule.offset == 0 ? 0 : 3600, rule.offset]),
        },
      },
    });
