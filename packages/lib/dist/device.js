...
const protocolFromPort_js_1 = require("./utils/protocolFromPort.js");
    ...
    async configureMQTTBrokersAndCredentials(mqtt, credentials) {
        ...
        const brokers = mqtt
            ...
            if (!URL.canParse(broker)) {
                ...
                if (port) {
                    const protocol = (0, protocolFromPort_js_1.protocolFromPort)(Number(port));
                    broker = `${protocol}://${broker}`;
                }
            }
            let { protocol, hostname, port } = new URL(broker);
            if (!port) {
                port = `${(0, protocolFromPort_js_1.portFromProtocol)(protocol.replace(':', ''))}`;
            }
            return {
                host: hostname,
                port: Number(port),
