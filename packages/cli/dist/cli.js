 ...
 */
export const percentToColor = (percent) => percent > 0.7 ? '^G' : percent > 0.5 ? '^Y' : percent > 0.3 ? '^y' : '^r';
/**
...
export const bar = (percent, width) => {
    ...
    return ((percentToColor(percent) + '▉').repeat(filled) +
...
export async function printDeviceTable(deviceInformation, deviceAbility, devicePassword) {
